...
| Method | Endpoint | Purpose |
|--------|----------|---------|
| GET | `/api/admin/diagnostics/summary` | Aggregates: LLM calls, tokens, agent counts, data transfers (24h) |
| GET | `/api/admin/diagnostics/requests` | Request log: last N with time, type, agent, tokens, latency, status |
